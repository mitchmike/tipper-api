...
class TestGameScrape(BaseScraperTest):
    ...
    def setUp(self):
        self.gameScrape = GameScraper(TestGameScrape._engine, 2019, 2019)
        ...
        self.game = self.gameScrape._populate_game(self.ONE_GAME_ROW, self.headers, self.YEAR, 1)
        # all rows test data
        self.games = self.gameScrape._process_row(self.first_row, self.headers, [], self.YEAR, 1)

    def test_process_row(self):
        one_game = self.gameScrape._process_row(self.one_row_soup, self.headers, [], self.YEAR, 1)
    ...
    def test_process_row_multiple(self):
        games = self.gameScrape._process_row(self.first_row, self.headers, [], self.YEAR, 1)
    ...
    def test_scrape_game(self):
        ...
        game_row = self.gameScrape._scrape_one_game(row)
    ...
    def test_scrape_game_populate_game_bad_links(self):
        ...
        self.assertEqual(game_row[4][1], '64')
        self.assertEqual(game_row[4][2], '97')
        game = self.gameScrape._populate_game(game_row, self.headers, self.YEAR, 1)
        self.assertIsNone(game.home_team)
        self.assertIsNone(game.away_team)

    def test_populate_game(self):
        game = self.gameScrape._populate_game(self.ONE_GAME_ROW, self.headers, self.YEAR, 1)
    ...
    def test_upsert_game_new(self):
        self.gameScrape._upsert_games([self.game], TestGameScrape._engine)
    ...
    def test_upsert_games_full_year(self):
        self.gameScrape._upsert_games(self.games, TestGameScrape._engine)
        with TestGameScrape.Session() as session:
